...
driver = ClosedLoopMotors(motors, encoders, kp=2.0, ki=10.0, kf=1.0)
...
def music_thread_func():
    ...
    while True:
        ...
        if flag:
            ...
            if buzzer.is_playing(): buzzer.off()
            time.sleep_ms(100)
...
def lqr_follow_step():
    ...
    if imu.gyro.data_ready():
        ...
        current_gyro_rad = (rad_z * GYRO_FILTER) + (current_gyro_rad * (1 - GYRO_FILTER))

    if line[1] < 700 and line[2] < 700 and line[3] < 700:
        if last_error < 0: pos = 0     
        else: pos = 4000  
    else:
        denom = sum(line)
        if denom == 0: denom = 1
        pos = (1000*line[1] + 2000*line[2] + 3000*line[3] + 4000*line[4]) // denom

    error = pos - 2000
    last_error = error
    
    if abs(error) > 1200:
        cmd_v = TARGET_VELOCITY_CM * 0.7
    else:
...
def lane_keep_step():
    ...
    if line[0] > 400:
        driver.car_move(v_cruise, -w_turn)
    elif line[4] > 400:
        driver.car_move(v_cruise, w_turn)
    else:
        driver.car_move(v_cruise, 0)
...
display.fill(0); display.text("Hands OFF!", 10, 20); display.show()
time.sleep_ms(1000)
...
led.off()
display.fill(0); display.text("Ready", 20, 25, 1); display.show()
